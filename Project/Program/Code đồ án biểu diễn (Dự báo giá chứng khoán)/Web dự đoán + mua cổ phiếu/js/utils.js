...
function histogram(arr, bins = 30, norm = true, density = false, jitter = 0.001) {
	...
	if (norm)
		for (var b = 0; b < arr_bins.length; b++) hist[b] /= arr.length
	if (density)
		for (var b = 0; b < arr_bins.length; b++) hist[b] /= (arr.length * bins)
	return {
		'y': hist,
		'x': x_arange
	}
...
function simple_investor(real_signal, predicted_signal, delay, initial_money, max_buy, max_sell, dates) {
	...
	return {
		'overall gain': (initial_money - starting_money),
		'overall investment': invest,
		'sell_Y': states_sell_Y,
		'sell_X': states_sell_X,
		'buy_Y': states_buy_Y,
		'buy_X': states_buy_X,
		'output': outputs
	}
